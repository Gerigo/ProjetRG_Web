...
if (isset($_GET['supprimer'])) {
    $commande = new CommandeDB($cnx);
     $commande_cli = $commande->getCommandeClient($_SESSION['mon_client']);
      $commande_client = $commande->getCommandeClient($_SESSION['mon_client']);
    $commande->delCommande(array("id_client" => $_SESSION['mon_client'], "id_jeu" => $_SESSION['id_jeu']));
    var_dump($_SESSION);
...
for ($i = 0; $i < sizeof($commande_client); $i++) {
    ?>
            <tr class="table-danger">
                <th scope="row"><?php print $commande_client[$i]['id_jeu'] ?></th>
                <td> <?php
        ...
        $cmp = $cmp + ($commande_client[$i]['prix']);
            ?></td>
                <td>  
        <a href="index.php?page=mon_panier&id=<?php print $commande_client[$i]['id_jeu']; ?>">
          Supprimer du panier
          </a>
